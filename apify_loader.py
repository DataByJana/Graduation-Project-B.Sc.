...
from .preprocessing import safe_get, normalize_date
...
def fetch_tweets(apify_token: str, max_items: int = 300, queries: List[str] = None) -> pd.DataFrame:
    """Fetch tweets from Apify actor apidojo/tweet-scraper and return normalized rows."""
    if not apify_token:
        return pd.DataFrame()
    ...
    # apidojo/tweet-scraper input fields can change; this payload supports the common search mode.
    run_input = {
        "searchTerms": queries,
    ...
    rows = []
    for item in items:
        text = safe_get(item, ["text", "full_text", "content", "tweetText"])
        rows.append({
            "source": "Twitter/X",
            "record_id": safe_get(item, ["id", "tweet_id", "url"]),
            "text": text,
            "created_at": normalize_date(safe_get(item, ["createdAt", "created_at", "date"])),
            "metadata_location": safe_get(item, ["place", "location"]) or _profile_location(item),
            "author": safe_get(item.get("author", {}) if isinstance(item.get("author"), dict) else {}, ["userName", "username", "name"]),
