...
def _extract_entities(entities: List[Dict]) -> Dict[str, str]:
    ...
    for ent in entities or []:
        ...
        if any(x in group for x in [
            "hazard", "haz", "pathogen", "contaminant",
            "microorganism", "symptom"
        ]):
        ...
        elif any(x in group for x in [
            "location", "loc", "gpe", "city",
            "state", "country", "place"
        ]):
...
def run_fda_pipeline(
    ...
    df["alert_label"] = "FDA Official Recall"
    df["alert_score"] = 1.0

    if "reason_for_recall" in df.columns:
        df["hazard"] = df["reason_for_recall"].apply(detect_hazard_fallback)
    else:
        df["hazard"] = "Food Safety Concern"

    if "product_description" in df.columns:
        df["product"] = df["product_description"].fillna("").replace("", "Unknown")
    else:
        df["product"] = "Unknown"
...
def run_pipeline(
    ...
    twitter_results = run_twitter_pipeline(
        df=tweets_df,
    ...
    fda_results = run_fda_pipeline(
        df=fda_df,
        category_pipe=category_pipe,
    )

    final_df = pd.concat(
        [twitter_results, fda_results],
        ignore_index=True,
        sort=False,
    )
